...
from gan_collections.log_utils import Logger

import gan_collections.source.losses as losses
from gan_collections.source.utils import generate_imgs, infiniteloop, set_seed
from gan_collections.source.gens import net_G_models
from gan_collections.source.discs import net_D_models
...
def generate(config):
    ...
    with torch.no_grad():
        for start in trange(
                0, config.gen.kimg * 1000, config.gen.batch, dynamic_ncols=True):
            batch_size = min(config.gen.batch, config.gen.kimg * 1000 - start)
...
def train(config):
    ...
    if config.data.dataset == 'stl10':
        dataset = datasets.STL10(
            model_config.dataset, split='unlabeled', download=True,
            transform=transforms.Compose([
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=config.gen.batch, shuffle=True, num_workers=config.gen.workers,
        drop_last=True)
    ...
    for step in pbar:
        ...
        for _ in range(model_config.n_dis):
            ...
            if config.gen.model == 'wgangp':
                ...
                loss = loss + model_config.alpha * loss_gp # loss_all
                wandb.log({"loss_dis": loss}) # loss all
        ...
        z = torch.randn(config.gen.batch * 2, model_config.z_dim).to(device)
        ...
        if step == 1 or step % config.log.eval_step == 0:
            ...
            if config.log.record:
                ...
                with open(os.path.join(config.log.logdir + f'/{config.exp.name}', "logs.txt"), 'a') as f:
                    ...
                    f.write(format_str)
                pbar.pritn_metrics(FID, IS)



def gan_collections_launch(config):
    # limit CPU usage
